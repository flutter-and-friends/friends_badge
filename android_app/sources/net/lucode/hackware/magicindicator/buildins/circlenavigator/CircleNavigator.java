...
public class CircleNavigator extends View implements IPagerNavigator {
    ...
    public void onPageScrolled(int i, float f, int i2) {
        ...
        int iMin = Math.min(this.mCirclePoints.size() - 1, i);
        int iMin2 = Math.min(this.mCirclePoints.size() - 1, i + 1);
        PointF pointF = this.mCirclePoints.get(iMin);
        this.mIndicatorX = pointF.x + ((this.mCirclePoints.get(iMin2).x - pointF.x) * this.mStartInterpolator.getInterpolation(f));
        invalidate();
    }
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (action == 0) {
            ...
            for (int i2 = 0; i2 < this.mCirclePoints.size(); i2++) {
                float fAbs = Math.abs(this.mCirclePoints.get(i2).x - x);
                if (fAbs < f) {
                    i = i2;
                    f = fAbs;
                }
