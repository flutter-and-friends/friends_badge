...
class EngineJob<R> implements DecodeJob.Callback<R>, FactoryPools.Poolable {
    ...
    void notifyCallbacksOfResult() {
        synchronized (this) {
            ...
            while (it.hasNext()) {
                ...
                next.executor.execute(new CallResourceReady(next.f330cb));
            }
    ...
    void notifyCallbacksOfException() {
        synchronized (this) {
            ...
            while (it.hasNext()) {
                ResourceCallbackAndExecutor next = it.next();
                next.executor.execute(new CallLoadFailed(next.f330cb));
            }
            decrementPendingCallbacks();
        }
    }
    ...
    private class CallLoadFailed implements Runnable {
        ...
        /* renamed from: cb */
        private final ResourceCallback f328cb;

        CallLoadFailed(ResourceCallback resourceCallback) {
            this.f328cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f328cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f328cb)) {
                        EngineJob.this.callCallbackOnLoadFailed(this.f328cb);
                    }
    ...
    private class CallResourceReady implements Runnable {
        ...
        /* renamed from: cb */
        private final ResourceCallback f329cb;

        CallResourceReady(ResourceCallback resourceCallback) {
            this.f329cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f329cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f329cb)) {
                        EngineJob.this.engineResource.acquire();
                        EngineJob.this.callCallbackOnResourceReady(this.f329cb);
                        EngineJob.this.removeCallback(this.f329cb);
                    }
                    EngineJob.this.decrementPendingCallbacks();
                }
            }
    ...
    static final class ResourceCallbackAndExecutor {
        ...
        /* renamed from: cb */
        final ResourceCallback f330cb;
        final Executor executor;

        ResourceCallbackAndExecutor(ResourceCallback resourceCallback, Executor executor) {
            this.f330cb = resourceCallback;
            this.executor = executor;
        }

        public boolean equals(Object obj) {
            if (obj instanceof ResourceCallbackAndExecutor) {
                return this.f330cb.equals(((ResourceCallbackAndExecutor) obj).f330cb);
            }
            return false;
        }

        public int hashCode() {
            return this.f330cb.hashCode();
        }
    }
