...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f68mX = new float[iMin];
        this.f69mY = new float[iMin];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < iMin; i2++) {
            pathMeasure.getPosTan((i2 * length) / (iMin - 1), fArr, null);
            this.f68mX[i2] = fArr[0];
            this.f69mY[i2] = fArr[1];
        }
        if (Math.abs(this.f68mX[0]) <= 1.0E-5d && Math.abs(this.f69mY[0]) <= 1.0E-5d) {
            int i3 = iMin - 1;
            if (Math.abs(this.f68mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f69mY[i3] - 1.0f) <= 1.0E-5d) {
                int i4 = 0;
                float f = 0.0f;
                while (i < iMin) {
                    float[] fArr2 = this.f68mX;
                    int i5 = i4 + 1;
                    float f2 = fArr2[i4];
                    if (f2 < f) {
        ...
        sb.append(this.f68mX[0]);
        sb.append(",");
        sb.append(this.f69mY[0]);
        sb.append(" end:");
        int i6 = iMin - 1;
        sb.append(this.f68mX[i6]);
        sb.append(",");
        sb.append(this.f69mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f68mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f68mX[i2]) {
                length = i2;
