...
import cn.highlight.work_card_write.greendao.p005db.DoodleBeanDao;
...
public class DoodleBeanDaoUse {
    ...
    public static void deleteAllByName(String str) throws SQLException {
        MyApplication.getDaoInstant().getDoodleBeanDao().queryBuilder().where(DoodleBeanDao.Properties.ImageName.m900eq(str), new WhereCondition[0]).buildDelete().executeDeleteWithoutDetachingEntities();
    }

    public static List<DoodleBean> queryAllByName(String str) {
        try {
            return MyApplication.getDaoInstant().getDoodleBeanDao().queryBuilder().where(DoodleBeanDao.Properties.ImageName.m900eq(str), new WhereCondition[0]).list();
        } catch (Exception e) {
            e.printStackTrace();
