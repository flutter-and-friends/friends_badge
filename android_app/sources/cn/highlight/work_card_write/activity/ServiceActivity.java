...
public class ServiceActivity extends BaseActivity {
    ...
    @BindView(C0498R.id.btn_connect)
    Button btnConnect;

    @BindView(C0498R.id.edit_address)
    EditText editAddress;

    @BindView(C0498R.id.iv_back)
    ImageView ivBack;

    @BindView(C0498R.id.tv_title_base)
    TextView tvTitleBase;

    @Override // cn.highlight.work_card_write.base.BaseActivity
    protected int setLayoutId() {
        return C0498R.layout.activity_service;
    }
    ...
    public void testConnect() {
        ApiUtil.getInstance().testConn(this, Tools.getText(this.editAddress), new ApiResult<Result>() { // from class: cn.highlight.work_card_write.activity.ServiceActivity.2
            ...
            public void onSuss(Result result) {
                if (result == null) {
                    ToastUtil.showToast(ServiceActivity.this.getApplicationContext(), ServiceActivity.this.getResources().getString(C0498R.string.requestMsg1));
                } else if (result.getStatus().equals("200")) {
                    ToastUtil.showToast(ServiceActivity.this.getApplicationContext(), ServiceActivity.this.getResources().getString(C0498R.string.request_success));
                    SpUtil.putString(ServiceActivity.this.getBaseContext(), Constants.APP_URL, Tools.getText(ServiceActivity.this.editAddress));
                } else {
                    result.getMsg();
                    ToastUtil.showToast(ServiceActivity.this.getApplicationContext(), ServiceActivity.this.getResources().getString(C0498R.string.request_fail));
                }
            }
