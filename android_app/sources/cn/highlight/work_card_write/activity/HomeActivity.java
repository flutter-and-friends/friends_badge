...
public class HomeActivity extends BaseActivity {
    ...
    @BindView(C0498R.id.imgCard)
    ImageView imgCard;

    @BindView(C0498R.id.imgHome)
    ImageView imgHome;

    @BindView(C0498R.id.imgMy)
    ImageView imgMy;

    @BindView(C0498R.id.linearCard)
    LinearLayout linearCard;

    @BindView(C0498R.id.linearContent)
    LinearLayout linearContent;

    @BindView(C0498R.id.linearHome)
    LinearLayout linearHome;

    @BindView(C0498R.id.linearMy)
    ...
    private void initUI() {
        this.imgHome.setImageResource(C0498R.mipmap.work_home1);
        this.imgCard.setImageResource(C0498R.mipmap.work_card);
        this.imgMy.setImageResource(C0498R.mipmap.work_my);
    }
    ...
    private void initFragment() {
        ...
        getSupportFragmentManager().beginTransaction().add(C0498R.id.linearContent, this.editFragment).add(C0498R.id.linearContent, this.cardFragment).add(C0498R.id.linearContent, this.myFragment).commitAllowingStateLoss();
    ...
    @OnClick({C0498R.id.linearHome, C0498R.id.linearCard, C0498R.id.linearMy})
    public void onViewClicked(View view) {
        int id = view.getId();
        if (id == C0498R.id.linearCard) {
            setUI(1);
            setFragment(1);
            Constants.cardShowPage = "模板";
        } else if (id == C0498R.id.linearHome) {
            setUI(0);
            setFragment(0);
        } else {
            if (id != C0498R.id.linearMy) {
                return;
            }
            setUI(2);
            setFragment(2);
    ...
    public void setUI(int i) {
        this.imgHome.setImageResource(C0498R.mipmap.work_home);
        this.imgCard.setImageResource(C0498R.mipmap.work_card);
        this.imgMy.setImageResource(C0498R.mipmap.work_my);
        if (i == 0) {
            this.imgHome.setImageResource(C0498R.mipmap.work_home1);
        } else if (i == 1) {
            this.imgCard.setImageResource(C0498R.mipmap.work_card1);
        } else {
            if (i != 2) {
                return;
            }
            this.imgMy.setImageResource(C0498R.mipmap.work_my1);
        }
    }
