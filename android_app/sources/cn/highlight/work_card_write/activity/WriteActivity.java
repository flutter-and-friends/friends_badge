...
import cn.highlight.p004tx.BaseTxManager;
...
public class WriteActivity extends BaseActivity {
    ...
    private Handler handler = new Handler() { // from class: cn.highlight.work_card_write.activity.WriteActivity.11
        ...
        public void handleMessage(Message message) {
            if (message.what == 0) {
                if (WriteActivity.this.tvProgress == null || !WriteActivity.this.tvProgress.getText().equals(WriteActivity.this.getResources().getString(C0498R.string.write_search_for_bluetooth_devices))) {
                    return;
                }
                WriteActivity.this.txManager.cancelScan();
                WriteActivity.this.isSearch = false;
                WriteActivity writeActivity = WriteActivity.this;
                writeActivity.showErrorDialog(writeActivity.getString(C0498R.string.write_no_device_found), WriteActivity.this.getString(C0498R.string.dialog_tip_info3), true);
            ...
            if (message.what == 2) {
                ...
                writeActivity2.addInfo(writeActivity2.getResources().getString(C0498R.string.write_send_finished));
                return;
            }
            if (message.what == 3) {
                WriteActivity.this.isNormal = true;
            ...
            if (message.what == 4) {
                ...
                writeActivity3.showErrorDialog(writeActivity3.getString(C0498R.string.write_send_error), WriteActivity.this.getString(C0498R.string.dialog_tip_info5), true);
                WriteActivity.this.exitProgress();
                WriteActivity.this.addInfo("");
                return;
            }
    ...
    protected int setLayoutId() {
        return C0498R.layout.activity_write;
    }
    ...
    private void initShake() {
        this.slideSwitch.setSwitchImage(BitmapFactory.decodeResource(getResources(), C0498R.mipmap.switch_on), BitmapFactory.decodeResource(getResources(), C0498R.mipmap.switch_off));
        this.slideSwitch.setSwitchImageSize(getResources().getDimensionPixelSize(C0498R.dimen.dp_60), getResources().getDimensionPixelSize(C0498R.dimen.dp_26));
    ...
    @OnClick({C0498R.id.imgBack, C0498R.id.imgHome, C0498R.id.btnSave, C0498R.id.btnWrite})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case C0498R.id.btnSave /* 2131296402 */:
                if (this.isRead || this.isSearch || this.isConnect || this.isStart || this.isWrite) {
                    showTipToast(getString(C0498R.string.write_error_hint1));
                    break;
                } else {
                ...
                break;
            case C0498R.id.btnWrite /* 2131296404 */:
                if (this.isRead || this.isSearch || this.isConnect || this.isStart || this.isWrite) {
                    showTipToast(getString(C0498R.string.write_error_hint1));
                    break;
                } else {
            ...
            case C0498R.id.imgBack /* 2131296583 */:
                if (this.isRead || this.isSearch || this.isConnect || this.isStart || this.isWrite) {
                    showTipToast(getString(C0498R.string.write_error_hint));
                    break;
                } else {
                    finishActivity();
                    break;
                }
            case C0498R.id.imgHome /* 2131296585 */:
                if (this.isRead || this.isSearch || this.isConnect || this.isStart || this.isWrite) {
                    showTipToast(getString(C0498R.string.write_error_hint));
                    break;
                    ...
                    showErrorDialog(getString(C0498R.string.dialog_tip), getString(C0498R.string.dialog_tip_info1), false);
                    break;
                }
        }
    }
    ...
    private void saveModelData(boolean z) throws Throwable {
        if (Constants.editSaveModel == null) {
            showTipToast(getString(C0498R.string.write_save_error));
            return;
        }
        if (!this.isImageSave) {
        ...
        showTipToast(getString(C0498R.string.write_save_success));
    ...
    private void checkPermission() {
        ...
        if (nfcAdapter == null || !nfcAdapter.isEnabled()) {
            sb.append(getString(C0498R.string.write_nfc));
            sb.append("、");
        }
        BluetoothAdapter bluetoothAdapter = this.bluetoothAdapter;
        if (bluetoothAdapter == null || !bluetoothAdapter.isEnabled()) {
            sb.append(getString(C0498R.string.write_bluetooth));
            sb.append("、");
        }
        LocationManager locationManager = this.locationManager;
        if (locationManager == null || (!locationManager.isProviderEnabled("gps") && !this.locationManager.isProviderEnabled("network"))) {
            sb.append(getString(C0498R.string.write_location));
            sb.append("、");
        }
        if (!Tools.isNull(sb.toString())) {
            showTipToast(sb.toString().substring(0, sb.toString().length() - 1) + getString(C0498R.string.write_not_available));
            return;
        }
        showStartNFCDialog();
        readNFC();
    ...
    private void showStartNFCDialog() {
        View viewInflate = LayoutInflater.from(this).inflate(C0498R.layout.dialog_start_nfc_item, (ViewGroup) null);
        TextView textView = (TextView) viewInflate.findViewById(C0498R.id.tvCancel);
        this.nfcDialog = new BottomSheetDialog(this, C0498R.style.CustomBottomSheetStyle);
    ...
    private void initBLETXReceive() {
        this.onDataReceivedTag = new OnDataReceivedTag() { // from class: cn.highlight.work_card_write.activity.-$$Lambda$WriteActivity$h5fuse2w8mW2LTE1hlzESuNBHIU
            @Override // cn.highlight.p004tx.listener.OnDataReceivedTag
    ...
    public /* synthetic */ void lambda$initBLETXReceive$15$WriteActivity(byte[] bArr) {
        ...
        if (c == 0) {
            if (strSubstring2.equals("00")) {
                this.isStart = false;
                addInfo(getResources().getString(C0498R.string.write_send_data));
            ...
            this.isStart = false;
            addInfo(getResources().getString(C0498R.string.write_unknown_Card));
    ...
    */
        ...
        try {
            if (!this.isRead && !this.isSearch && !this.isConnect && !this.isStart && !this.isWrite) {
                ...
                if (!str2.equals(this.writeSize)) {
                    ...
                    dismissNFCDialog();
                    clearNFCCallback();
                    showErrorDialog(getString(C0498R.string.write_error_hint2), getString(C0498R.string.dialog_tip_info2), true);
                    return;
                }
                setImageData();
    ...
    private void serachBLE(Tag tag) {
        ...
        addInfo(getResources().getString(C0498R.string.write_search_for_bluetooth_devices));
        this.isSearch = true;
        ...
        this.txManager.startScanBle(new BleScanCallback() { // from class: cn.highlight.work_card_write.activity.WriteActivity.1
            @Override // cn.highlight.p004tx.ble.callback.BleScanPresenterImp
            public void onScanStarted(boolean z) {
            }

            @Override // cn.highlight.p004tx.ble.callback.BleScanCallback
    ...
    public void connectBLE(BleDevice bleDevice) {
        addInfo(getResources().getString(C0498R.string.write_connecting_devices));
        this.isConnect = true;
        this.txManager.conn(bleDevice, new OnConnStatus() { // from class: cn.highlight.work_card_write.activity.WriteActivity.2
            @Override // cn.highlight.p004tx.listener.OnConnStatus
            public void connSuccess() {
                WriteActivity.this.isSearch = false;
                WriteActivity.this.isConnect = true;
                WriteActivity writeActivity = WriteActivity.this;
                writeActivity.addInfo(writeActivity.getResources().getString(C0498R.string.write_connection_successful));
            ...
            public void connFail() {
                ...
                WriteActivity writeActivity = WriteActivity.this;
                writeActivity.showErrorDialog(writeActivity.getString(C0498R.string.write_connection_failed), WriteActivity.this.getString(C0498R.string.write_connection_failed), true);
            ...
            public void disConnected() {
                ...
                if (!WriteActivity.this.isNormal) {
                    ...
                    WriteActivity writeActivity = WriteActivity.this;
                    writeActivity.showErrorDialog(writeActivity.getString(C0498R.string.write_connection_disconnected), WriteActivity.this.getString(C0498R.string.dialog_tip_info4), true);
    ...
    public /* synthetic */ void lambda$sendStatrData$17$WriteActivity(byte[] bArr) {
        this.txManager.writeData(bArr, new BleWriteCallbackHighlight() { // from class: cn.highlight.work_card_write.activity.WriteActivity.3
            @Override // cn.highlight.p004tx.ble.callback.BleWriteCallbackHighlight
    ...
    private void CheckData() {
        ...
        this.txManager.writeData(new byte[]{-91, 0, 19, 19}, new C06069());
    }

    /* renamed from: cn.highlight.work_card_write.activity.WriteActivity$9 */
    class C06069 extends BleWriteCallbackHighlight {
        C06069() {
        }
    ...
    private void sendEndData() {
        ...
        this.txManager.writeData(new byte[]{-91, 0, 20, 20}, new C059610());
        refreshUI(0);
    }

    /* renamed from: cn.highlight.work_card_write.activity.WriteActivity$10 */
    class C059610 extends BleWriteCallbackHighlight {
        C059610() {
        }
    ...
    private void sendData(IsoDep isoDep) throws InterruptedException, IOException {
        try {
            ...
            showErrorDialog(getString(C0498R.string.write_send_error), getString(C0498R.string.dialog_tip_info5), true);
            this.isWrite = false;
            exitProgress();
            addInfo("");
            stopSwipe();
    ...
    public void swipe() {
        ...
        this.countDownTimer = new CountDownTimer(18000L, 1000L) { // from class: cn.highlight.work_card_write.activity.WriteActivity.12
            ...
            public void onTick(long j) {
                WriteActivity writeActivity = WriteActivity.this;
                writeActivity.addInfo(String.format(writeActivity.getString(C0498R.string.write_send_finished1), Long.valueOf(j / 1000)));
            }
