package cn.highlight.p004tx.ble;
...
import cn.highlight.p004tx.ble.bluetooth.BleBluetooth;
import cn.highlight.p004tx.ble.bluetooth.MultipleBluetoothController;
import cn.highlight.p004tx.ble.bluetooth.SplitWriter;
...
import cn.highlight.p004tx.ble.callback.BleScanAndConnectCallback;
...
import cn.highlight.p004tx.ble.data.BleScanState;
import cn.highlight.p004tx.ble.exception.OtherException;
import cn.highlight.p004tx.ble.scan.BleScanRuleConfig;
import cn.highlight.p004tx.ble.scan.BleScanner;
...
public class BleManager {
    ...
    public void scan(BleScanCallback bleScanCallback) {
        ...
        if (!isBlueEnable()) {
            BleLog.m41e("Bluetooth not enable!");
    ...
    public BluetoothGatt connect(BleDevice bleDevice, BleGattCallback bleGattCallback) {
        ...
        if (Looper.myLooper() == null || Looper.myLooper() != Looper.getMainLooper()) {
            BleLog.m43w("Be careful: currentThread is not MainThread!");
        }
    ...
    public void write(BleDevice bleDevice, String str, String str2, byte[] bArr, boolean z, boolean z2, long j, BleWriteCallbackHighlight bleWriteCallbackHighlight) {
        ...
        if (bArr == null) {
            BleLog.m41e("data is Null!");
            bleWriteCallbackHighlight.onWriteFailure(new OtherException("data is Null!"));
            return;
        }
        if (bArr.length > 20 && !z) {
            BleLog.m43w("Be careful: data's length beyond 20! Ensure MTU higher than 23, or use spilt write!");
        }
        BleBluetooth bleBluetooth = this.multipleBluetoothController.getBleBluetooth(bleDevice);
        if (bleBluetooth == null) {
