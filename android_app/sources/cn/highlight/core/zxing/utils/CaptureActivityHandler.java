...
public class CaptureActivityHandler extends Handler {
    ...
    public void handleMessage(Message message) {
        if (message.what == C0450R.id.restart_preview) {
            restartPreviewAndDecode();
            return;
        }
        if (message.what == C0450R.id.decode_succeeded) {
            this.state = State.SUCCESS;
        ...
        if (message.what == C0450R.id.decode_failed) {
            this.state = State.PREVIEW;
            this.cameraManager.requestPreviewFrame(this.decodeThread.getHandler(), C0450R.id.decode);
        } else if (message.what == C0450R.id.decode_error) {
            this.scanManager.handleDecodeError((Exception) message.obj);
        } else {
            int i = message.what;
            int i2 = C0450R.id.return_scan_result;
        }
    }
    ...
    public void quitSynchronously() {
        ...
        Message.obtain(this.decodeThread.getHandler(), C0450R.id.quit).sendToTarget();
        try {
            this.decodeThread.join(500L);
        } catch (InterruptedException unused) {
        }
        removeMessages(C0450R.id.decode_succeeded);
        removeMessages(C0450R.id.decode_failed);
    }
