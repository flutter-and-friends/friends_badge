...
public class ScanManager implements SurfaceHolder.Callback {
    ...
    public void surfaceCreated(SurfaceHolder surfaceHolder) {
        if (surfaceHolder == null) {
            LogManage.m31e(this.TAG, "*** WARNING *** surfaceCreated() gave us a null surface!");
        }
        if (this.isHasSurface) {
            return;
        }
    ...
    void initCamera(SurfaceHolder surfaceHolder) {
        ...
        if (this.cameraManager.isOpen()) {
            LogManage.m38w(this.TAG, "initCamera() while already open -- late SurfaceView callback?");
            return;
        }
        try {
            ...
            if (this.handler == null) {
                ...
                LogManage.m31e("hongliang1", "handler new成功！:" + this.handler);
            }
            initCrop();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void reScan() {
        CaptureActivityHandler captureActivityHandler = this.handler;
        if (captureActivityHandler != null) {
            captureActivityHandler.sendEmptyMessage(C0450R.id.restart_preview);
        }
    }
