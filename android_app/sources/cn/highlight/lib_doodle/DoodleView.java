...
public class DoodleView extends FrameLayout implements IDoodle {
    ...
    public DoodleView(Context context, Bitmap bitmap, boolean z, IDoodleListener iDoodleListener, IDoodleTouchDetector iDoodleTouchDetector) {
        ...
        if (bitmap2.getConfig() != Bitmap.Config.RGB_565) {
            LogUtil.m25w(TAG, "the bitmap may contain alpha, which will cause eraser don't work well.");
        }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (hasFlag(2)) {
            LogUtil.m13d(TAG, "FLAG_RESET_BACKGROUND");
            clearFlag(2);
            clearFlag(4);
            clearFlag(8);
            ...
            LogUtil.m13d(TAG, "FLAG_DRAW_PENDINGS_TO_BACKGROUND");
            clearFlag(4);
            clearFlag(8);
            ...
            LogUtil.m13d(TAG, "FLAG_REFRESH_BACKGROUND");
            clearFlag(8);
            this.mBackgroundView.invalidate();
        }
    ...
    private class BackgroundView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            if (LogUtil.sIsLog) {
                LogUtil.m13d(DoodleView.TAG, "BackgroundView>>onDraw");
            }
            int iSave = canvas.save();
    ...
    private class ForegroundView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            if (LogUtil.sIsLog) {
                LogUtil.m13d(DoodleView.TAG, "ForegroundView>>onDraw");
            }
            int iSave = canvas.save();
            canvas.rotate(DoodleView.this.mDoodleRotateDegree, getWidth() / 2, getHeight() / 2);
            doDraw(canvas);
