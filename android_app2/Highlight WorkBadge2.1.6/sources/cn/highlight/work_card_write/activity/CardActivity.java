...
public class CardActivity extends BaseActivity {
    ...
    protected void initViews(Bundle bundle) {
        this.tvNum.setText(getString(R.string.card_theme_selected, new Object[]{"0"}));
    ...
    public /* synthetic */ void lambda$refreshUI$1$CardActivity() {
        ...
        this.tvNum.setText(getString(R.string.card_theme_selected, new Object[]{this.selectList.size() + ""}));
        if (this.dataList.size() == this.selectList.size()) {
            this.tvSelect.setText(getString(R.string.card_theme_unselect));
        } else {
            this.tvSelect.setText(getString(R.string.card_theme_select));
        }
        if (this.selectList.size() > 0) {
            this.imageCopy.setBackgroundResource(R.mipmap.copy1);
            this.tvCopy.setTextColor(getResources().getColor(R.color.text_color_3));
            if (this.dataList.size() > 1 && this.dataList.size() == this.selectList.size()) {
                this.imageCopy.setBackgroundResource(R.mipmap.copy);
                this.tvCopy.setTextColor(getResources().getColor(R.color.text_color_6));
            }
        } else {
            this.imageCopy.setBackgroundResource(R.mipmap.copy);
            this.tvCopy.setTextColor(getResources().getColor(R.color.text_color_6));
        ...
        if (this.selectList.size() > 0) {
            this.imageDelete.setBackgroundResource(R.mipmap.delete2);
            this.tvDelete.setTextColor(getResources().getColor(R.color.text_color_3));
        } else {
            this.imageDelete.setBackgroundResource(R.mipmap.delete1);
            this.tvDelete.setTextColor(getResources().getColor(R.color.text_color_6));
        }
    }

    @OnClick({R.id.linearCancel, R.id.tvSelect, R.id.linearCopy, R.id.linearDelete})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.linearCancel /* 2131296633 */:
                finishActivity();
                break;
            case R.id.linearCopy /* 2131296637 */:
                if (this.selectList.size() > 0) {
                    if (this.dataList.size() <= 1 || this.dataList.size() != this.selectList.size()) {
                        showDialog(getResources().getString(R.string.card_theme_copy_sure), "copy");
                        break;
                    }
                }
                break;
            case R.id.linearDelete /* 2131296639 */:
                if (this.selectList.size() > 0) {
                    showDialog(getResources().getString(R.string.card_theme_delete_sure), "delete");
                    break;
                }
                break;
            case R.id.tvSelect /* 2131296940 */:
                if (Tools.getText(this.tvSelect).equals(getString(R.string.card_theme_select))) {
    ...
    public /* synthetic */ void lambda$showDialog$5$CardActivity(String str, final String str2) {
        ...
        TextView textView = (TextView) viewInflate.findViewById(R.id.tv_info);
