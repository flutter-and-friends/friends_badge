...
import cn.highlight.work_card_write.greendao.db.BadgeBeanDao;
...
public class BadgeBeanDaoUse {
    ...
    public static List<BadgeBean> queryAllBySize(String str) {
        try {
            return MyApplication.getDaoInstant().getBadgeBeanDao().queryBuilder().where(BadgeBeanDao.Properties.Size.eq(str), new WhereCondition[0]).list();
        } catch (Exception e) {
            e.printStackTrace();
